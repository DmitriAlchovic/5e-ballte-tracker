...
const BattleCard: React.FC<BattleCardProps> = ({
  ...
  const [char] = useState<Character>(charArray[charIndex]);
      ...
      <Card border="primary" >
        ...
        <Card.Body>
          ...
          <Card.Text>
            ...
            <input
              min={0}
              onChange={(event) => hitpointsChange(event, charIndex)}
              className="hit-points-input"
              value={hitPointsArr[charIndex]}
              type="number"
            />
          </Card.Text>
          <Card.Text>
          ...
          <Table>
            ...
            <tbody>
              <tr>
                ...
                <th>
                  {char.constitution
                    ? `${char.constitution} (${getStatBonus(
                        char.constitution
                      )})`
                    : null}
                </th>
                <th>
                  {char.intelligence
                    ? `${char.intelligence} (${getStatBonus(
                        char.intelligence
                      )})`
                    : null}
                </th>
          ...
          {char.damageImmunities?.length ? (
            <Card.Text>Damage Immunities:{char.damageImmunities}</Card.Text>
          ) : null}
          {char.damageResistances?.length ? (
            <Card.Text>Damage Resistances:{char.damageResistances}</Card.Text>
          ) : null}
          {char.damageVulnerabilities?.length ? (
            <Card.Text>
              Damage Vulnerabilities:{char.damageVulnerabilities}
            </Card.Text>
          ) : null}
          <Card.Text>
            Senses:{" "}
        ...
        {char.legendaryActions
          ? displayActionsList(char.legendaryActions)
          : null}
      </Card>
    </div>
  );
